...
def mostrar_pagina_svr():
    # Título de la aplicación
    st.title("Modelo SVR para Predicción de Precios de Acciones")

    # Descripción de la aplicación
    st.write("""
    Esta aplicación permite realizar un análisis de predicción de precios de acciones utilizando un modelo SVR.
    ...
    **Rango de fechas:** Selecciona el rango de fechas para el cual deseas realizar el análisis.
    ...
    # Preparar los datos para el modelo SVR
    ...
    mape_svr = mean_absolute_percentage_error(y_test, predictions)
    rmse_svr = np.sqrt(mean_squared_error(y_test, predictions))

    st.write(f"**MAPE del modelo SVR:** {mape_svr:.4f}")
    st.write(f"**RMSE del modelo SVR:** {rmse_svr:.4f}")
